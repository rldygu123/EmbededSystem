...
int compute(char ent1,int i1,int j1,int k1,int l1,int cond,char chess[8][8]);

void printing(char chp[8][8]);

void PawnAI(char swap[8][8], char side, int i,int j, node* root);
void KnightAI(char swap[8][8], char side, int i, int j, node* root);
void BishopAI(char swap[8][8],char side, int i, int j, node* root);
void RookAI(char swap[8][8], char side, int i, int j, node* root);
void QueenAI(char swap[8][8], char side, int i, int j, node* root);
void KingAI(char swap[8][8], char side, int i, int j, node* root);
void common_init(int i1, int j1, int k1, int l1, char swap[8][8], node *root,char play_side);


void assign_ent(char side, node* branch, char swap[8][8])
{
    int i,j=0;
...
void common_init(int i1, int j1, int k1, int l1, char swap[8][8], node *root,char play_side)
{
    char init_side = swap[i1][j1] % 10;
    char init_ent  = swap[i1][j1] / 10;
    char finl_side = swap[k1][l1] % 10;
    char finl_ent  = swap[k1][l1] / 10;
    ...
    /*  printf("eval=%d\n\n", branch->elist->eval);*/
      swap[i1][j1] / 10 == init_ent;      /* this is      */
      swap[i1][j1] % 10 == init_side;     /* for undoing  */
      swap[k1][l1] / 10 == finl_ent;      /* the changes  */
      swap[k1][l1] % 10 == finl_side;     /* in swap      */
...
void AIfunc(char origin[8][8])
{
 /*   printf("origin starts\n");
    ...
    for(a; a<=7; a++)
    {
      ...
      for(b; b<=7; b++)
      {
          copy[a][b] / 10 == origin[a][b] / 10;
          copy[a][b] % 10 == origin[a][b] % 10;
          
          swap[a][b] / 10 == origin[a][b] / 10;
          swap[a][b] % 10 == origin[a][b] % 10;
      };
    };
/*    printing(swap);*/
...
/*    printing(swap);*/
    ...
    for(z;z<=99; z++)
    {
       if(root->array[z]!=NULL)
       {
         ...
         init_ent  = swap[i1][j1] / 10;
         init_side = swap[i1][j1] % 10;
         finl_ent  = swap[k1][l1] / 10;
         finl_side = swap[k1][l1] % 10;

         int qw = compute(init_ent,i1,j1,k1,l1,1,swap);
         assign_ent('1',branch,swap);

         swap[i1][j1] / 10 == init_ent;
         swap[i1][j1] % 10 == init_side;
         swap[k1][l1] / 10 == finl_ent;
         swap[k1][l1] % 10 == finl_side;
       }
    ...
    {                            /*which is a threat.*/
       ...
       {
          shell = root->array[c];
          if(shell!=NULL && copy[shell->elist->i][shell->elist->j] / 10 == '9' && shell->dcntr==0)
          {
